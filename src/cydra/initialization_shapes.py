...
def _fallback_shape(function_model: FunctionModel, target_var: str, unauthorized_addr: str, initialize_args_str: str) -> str:
    ...
    return (
        ...
        "    vm.record();\n"
        "    vm.prank(unauthorized);\n"
        f"    (bool ok,) = address({target_var}).call(abi.encodeWithSelector({target_var}.{function_model.name}.selector, {initialize_args_str}));\n"
        f"    (bytes32[] memory reads, bytes32[] memory writes) = vm.accesses(address({target_var}));\n"
        "    reads;\n"
        "    assertTrue(!ok || writes.length == 0, \"arbitrary initializer call mutated target storage\");\n"
        "}"
    )
