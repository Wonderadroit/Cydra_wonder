...
_PATTERNS = (
    (
        re.compile(
            r"\b(?P<state>[A-Za-z_]\w*)\s*\+=\s*(?P<rhs>" + _RHS + r")\s*;"
        ),
        "+",
    ),
    (
    ...
    # Explicit self-referential scalar assignments are equivalent to +=/-=.
    (
        re.compile(
            r"\b(?P<state>[A-Za-z_]\w*)\s*=\s*(?P=state)\s*\+\s*(?P<rhs>"
            + _RHS + r")\s*;"
        ),
        "+",
    ),
    ...
    # The exact same key expression must appear on both sides.
    (
        re.compile(
            r"\b(?P<state>[A-Za-z_]\w*)(?P<indexes>(?:\[[^\]]+\])+)"
            r"\s*=\s*(?P=state)(?P=indexes)\s*\+\s*(?P<rhs>"
            + _RHS + r")\s*;"
        ),
        "+",
    ),
...
def _function_body(source: str, function: FunctionModel) -> str:
    ...
    matches = list(
        re.finditer(r"\bfunction\s+" + re.escape(function.name) + r"\s*\(", source)
    )
    if not matches:
        return ""
    target = min(
...
def _index_expressions(raw: str) -> tuple[str, ...] | None:
    ...
    if not expressions or any(
        item != "msg.sender" and not re.fullmatch(r"[A-Za-z_]\w*", item)
