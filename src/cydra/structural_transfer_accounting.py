...
def _transfer_argument(body):
    for match in re.finditer(r"\.(?:safeTransferFrom|transferFrom)\s*\(([^)]*)\)", body):
        ...
        if len(arguments) >= 3 and re.fullmatch(r"[A-Za-z_]\w*", arguments[-1]):
            return arguments[-1]
    return None
...
def generate_transfer_accounting_hypotheses(contract: ContractModel, semantic=()):
    ...
    for f in contract.functions:
        ...
        if not body or "transferFrom" not in body or not amount or not _credits_requested_amount(body, amount, contract.state_variables) or _measures_delta(body): continue
