...
def test_planned_inputs_override_initializer_fallback_arguments(tmp_path):
    ...
    source = output.read_text(encoding="utf-8")
    assert "(bool ok,) = address(target).call(abi.encodeWithSelector(target.initialize.selector, 7, address(0xCAFE)));" in source
...
def test_model_aware_generator_qualifies_and_imports_inherited_custom_type(tmp_path):
    ...
    assert "(bool ok,) = address(target).call(abi.encodeWithSelector(target.initialize.selector, parameter0));" in source
