...
from cydra.models import ContractModel, ConstructorModel, FunctionModel, Hypothesis, ParameterModel
from cydra.structural_auth_execution import generate_structural_authorization_test


def _model(parameters=(), constructor_parameters=()):
    return ContractModel(
        ...
        constructor=(
            ConstructorModel(tuple(constructor_parameters), 5)
...
def test_generation_supports_common_constructor_arguments(tmp_path: Path):
    model = _model(constructor_parameters=(ParameterModel("governance", "address"),))
    path = generate_structural_authorization_test(
        _hypothesis(), model, "../Target.sol", model.name, tmp_path / "generated.t.sol"
    )
    source = path.read_text(encoding="utf-8")
    assert "target = new RenamedTarget(address(0xCAFE));" in source


def test_generation_rejects_unresolved_constructor_custom_type(tmp_path: Path):
    model = _model(constructor_parameters=(ParameterModel("governance", "Governance"),))
    with pytest.raises(ValueError, match="unsupported structural authorization argument type"):
        generate_structural_authorization_test(
            _hypothesis(), model, "../Target.sol", model.name, tmp_path / "generated.t.sol"
        )


def test_generation_rejects_unresolved_custom_function_type(tmp_path: Path):
