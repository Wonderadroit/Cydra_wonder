...
def test_executed_differential_establishes_hypothesis_and_unlocks_finding_gate():
    ...
    result = run_canonical_differential_cycle(
        model, hypothesis=Hypothesis("h1", "unauthorized caller can mutate privileged state"), observation_id="o1",
        vulnerable=execution("vulnerable", "vulnerable", "FAIL"), patched=execution("patched", "patched", "PASS"), outcome_id="cycle-1",
    )
    ...
    gate = evaluate_finding_graph(
        model, candidate=FindingCandidate(True, False, True, True, True, True), finding_id="finding:1",
        hypothesis_id="hypothesis:h1", evidence_ids=result.causal_verification.evidence_ids, causal_chain_id=result.causal_chain.chain_id,
    )
...
def test_non_differential_execution_does_not_establish_hypothesis():
    ...
    result = run_canonical_differential_cycle(
        model, hypothesis=Hypothesis("h1", "unauthorized caller can mutate privileged state"), observation_id="o1",
        vulnerable=execution("vulnerable", "vulnerable", "PASS"), patched=execution("patched", "patched", "PASS"), outcome_id="cycle-2",
    )
...
def test_reversed_differential_contradicts_hypothesis_without_unlocking_finding_gate():
    ...
    result = run_canonical_differential_cycle(
        model, hypothesis=Hypothesis("h1", "unauthorized caller can mutate privileged state"), observation_id="o1",
        vulnerable=execution("vulnerable", "vulnerable", "PASS"), patched=execution("patched", "patched", "FAIL"), outcome_id="cycle-3",
    )
