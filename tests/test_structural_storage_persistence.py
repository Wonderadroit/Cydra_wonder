...
def _planner(h):
    return Experiment("EXP-" + h.hypothesis_id, h.hypothesis_id, "persist", ("state persists",), 1.0)
...
def test_memory_alias_storage_mutation_generates_hypothesis(tmp_path):
    ...
    assert any(h.invariant_id.startswith("INV-STORAGE-PERSISTENCE-") for h in result.hypotheses)
...
def test_existing_surfaces_do_not_recognize_storage_persistence_shape(tmp_path):
    ...
    assert not any(h.invariant_id.startswith("INV-STORAGE-PERSISTENCE-") for h in result.hypotheses)
