...
def test_reasoning_pipeline_promotes_only_supported_invariants():
    ...
    hypotheses = verify_and_bind_invariants(model, candidates, (VerificationEvidence("e1", VerificationRole.SUPPORTS, 0.9),))
    assert [item.invariant_id for item in hypotheses] == ["candidate:supported", "candidate:unresolved"]
