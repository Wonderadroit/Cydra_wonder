...
def test_invariant_candidates_are_derived_from_evidence_backed_graph_edges():
    ...
    model.connect("function:C:f", "writes", "state:C:x", evidence_backed=True, candidate=True,
                  provenance="compiler", confidence=0.9, ast_node_id=12)
    candidates = derive_invariant_candidates(model)
    assert len(candidates) == 1
    assert candidates[0].source_ids == ("compiler:12",)


def test_belief_update_preserves_explicit_support_and_changes_no_execution_state():
