...
def main():
 ...
 m.add_node(Node(cid,"contract",result.contracts[0].name,{}));m.add_node(Node(fid,"function",h.target_function,{}));m.add_node(Node(iid,"invariant",h.claim,{"status":"inferred"}));m.add_node(Node(hid,"hypothesis",h.claim,{"belief":0.5}));m.add_node(Node(f"observation:{oid}","observation","precondition-before-external-call",{"status":"planned","hypothesis_id":hid,"target_function_id":fid,"binding_status":"bound","experiment_binding":{"hypothesis_id":hid,"observation_id":f"observation:{oid}","target_function_id":fid}}));m.add_edge(Edge(iid,"informs",hid,{}));m.add_edge(Edge(f"observation:{oid}","tests",hid,{}))
