...
def canonical_model(hypothesis, contract_name):
    ...
    model.add_node(Node(oid,"observation","Execute the withdrawal at a fractional share boundary.",{"status":"planned","provenance":"structural redemption-rounding experiment"}))
    model.add_edge(Edge(iid,"informs",hid,{"provenance":"structural redemption-rounding reasoning"}))
    model.add_edge(Edge(oid,"targets",iid,{"provenance":"structural redemption-rounding reasoning"}))
    model.add_edge(Edge(oid,"tests",hid,{"provenance":"structural redemption-rounding reasoning"}))
