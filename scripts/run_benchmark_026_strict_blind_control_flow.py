...
contract CydraControlFlowStrictBlindTest is Test {
    Prime prime;
...
def write_harness(root: Path) -> None:
    (root / "contracts" / "test").mkdir(parents=True, exist_ok=True)
    (root / "contracts" / "test" / "CydraControlFlowStrictBlind.t.sol").write_text(TEST_SOURCE, encoding="utf-8")
...
def run_target(root: Path, label: str) -> dict:
    ...
    return {
        "experiment_id": f"X-STRICT-CONTROL-FLOW-{label}",
        "executed": True,
        "tests_run": 1,
...
def patch_target(root: Path) -> None:
    ...
    new = """            if (isScoreUpdated[nextScoreUpdateRoundId][user]) {
                unchecked {
                    i++;
                }
                continue;
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("backtest-artifacts/strict-blind-control-flow"))
    args = parser.parse_args()

    with tempfile.TemporaryDirectory(prefix="cydra-venus-strict-v-") as tmp:
        target = clone_target(Path(tmp) / "target")
        source = target / TARGET_PATH

        # Strict blind boundary: CYDRA receives only the target source. No class,
        # target function, state surface, reasoning-surface injection, or planner.
        ...
        if not hypothesis.invariant_id.startswith("INV-CONTROL-FLOW-"):
            raise SystemExit(
                ...
                + " instead of the control-flow hypothesis"
            )
        vulnerable = run_target(target, "vulnerable")

    with tempfile.TemporaryDirectory(prefix="cydra-venus-strict-p-") as tmp:
        target = clone_target(Path(tmp) / "target")
        patch_target(target)
        patched = run_target(target, "patched")

    with tempfile.TemporaryDirectory(prefix="cydra-venus-strict-repro-v-") as tmp:
        target = clone_target(Path(tmp) / "target")
        independent_vulnerable = run_target(target, "independent-vulnerable")

    with tempfile.TemporaryDirectory(prefix="cydra-venus-strict-repro-p-") as tmp:
        target = clone_target(Path(tmp) / "target")
        patch_target(target)
    ...
    cycle = run_canonical_differential_cycle(
        ...
        outcome_id="strict-control-flow-differential",
    )
    ...
    gate = evaluate_finding_graph(
        ...
        finding_id=f"F-STRICT-CONTROL-FLOW-{hypothesis.target_function}",
        hypothesis_id=f"hypothesis:{hypothesis.hypothesis_id}",
    ...
    payload = {
        ...
        "blind_selection": {"hypothesis_id": hypothesis.hypothesis_id, "target_function": hypothesis.target_function,
                            "score": selection.score, "candidate_count": len(result.hypotheses)},
        "hypothesis": hypothesis.__dict__, "experiment": experiment.__dict__,
        ...
        "boundary": "actual pinned Venus repository and dependency graph executed; causal control changes only loop progress.",
    }
