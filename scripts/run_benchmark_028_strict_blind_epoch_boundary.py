...
def clone_target(destination: Path) -> Path:
    ...
    subprocess.run(("git", "-C", str(destination), "checkout", "--detach", TARGET_REF),
                   check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    subprocess.run(("git", "-C", str(destination), "submodule", "update", "--init", "--recursive"),
                   check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    return destination

def run_target(root: Path, label: str):
    path = root / "src" / "test" / "CydraEpochBoundary.t.sol"
    ...
    return {
        ...
        "executed": True, "tests_run": 1,
        "tests_failed": 0 if completed.returncode == 0 else 1,
        "status": "PASS" if completed.returncode == 0 else "FAIL",
        "exit_code": completed.returncode, "stdout": completed.stdout[-12000:], "stderr": "",
    }
...
def canonical_model(hypothesis):
    ...
    model.add_node(Node("contract:LendingLedger", "contract", "LendingLedger",
    ...
    model.add_node(Node(invariant_id, "invariant", hypothesis.claim,
                        {"status": "inferred", "confidence": 0.86}))
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("backtest-artifacts/strict-blind-epoch-boundary"))
    args = parser.parse_args()

    with tempfile.TemporaryDirectory(prefix="cydra-canto-blind-v-") as tmp:
        target = clone_target(Path(tmp) / "target")
        source = target / TARGET_PATH

        # Strict blind boundary: only the pinned source enters the normal pipeline.
    ...
    with tempfile.TemporaryDirectory(prefix="cydra-canto-blind-repro-v-") as tmp:
        target = clone_target(Path(tmp) / "target")
        independent_vulnerable = run_target(target, "independent-vulnerable")

    with tempfile.TemporaryDirectory(prefix="cydra-canto-blind-repro-p-") as tmp:
        target = clone_target(Path(tmp) / "target")
        patch_target(target)
    ...
    cycle = run_canonical_differential_cycle(
        ...
        outcome_id="strict-blind-epoch-boundary-differential",
    )
    ...
    gate = evaluate_finding_graph(
        ...
        finding_id=f"F-STRICT-BLIND-EPOCH-ACCOUNTING-{hypothesis.target_function}",
        hypothesis_id=f"hypothesis:{hypothesis.hypothesis_id}",
    ...
    payload = {
        ...
        "strict_blind_selection": {"hypothesis": hypothesis.__dict__, "score": selection.score},
        ...
        "boundary": "normal class-neutral pipeline selected the hypothesis from the pinned source; causal control changes only the segment-end calculation; independent clones reproduce the differential.",
    }
