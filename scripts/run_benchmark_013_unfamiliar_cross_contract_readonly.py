...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="cydra-blueberry-") as tmp:
        ...
        model.add_node(Node(oid, "observation", "external state-derived reads differ during a callback", {"status": "planned", "hypothesis_id": hid, "binding_status": "bound", "experiment_binding": {"hypothesis_id": hid, "observation_id": oid, "target_function_id": f"function:BalancerPairOracle:{hypothesis.target_function}", "experiment_id": experiment.experiment_id}}))
