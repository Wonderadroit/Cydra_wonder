...
def run_mode(mode: str, target: Path, output: Path):
    ...
    for hypothesis in result.hypotheses:
        ...
        try:
            ...
            generated = test_path_for(
                workspace, f"test/generated/{hypothesis.hypothesis_id}.t.sol"
            )
            if hypothesis.invariant_id.startswith("INV-STATE-"):
                ...
                row["execution"] = {
                    "status": "not_rendered",
                    "reason": "no generic renderer for this hypothesis",
                }
                rows.append(row)
                continue
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cydra-target-") as temp:
        ...
        run_mode(args.mode, checkout / PATH, args.output)
