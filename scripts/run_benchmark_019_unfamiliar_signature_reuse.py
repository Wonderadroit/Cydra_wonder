...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cydra-phi-vulnerable-") as tmp:
        ...
        result = investigate(
            ...
            reasoning_surfaces=(generate_signature_reuse_hypotheses,),
            experiment_planner=plan_signature_reuse_experiment,
        )
        hypotheses = [h for h in result.hypotheses if h.invariant_id.startswith("INV-SIGNATURE-REUSE-")]
        if len(hypotheses) != 1:
            raise SystemExit(f"expected one blind signature-reuse hypothesis, got {len(hypotheses)}")
