...
## Configured runtime-dependency checkpoint
The execution-readiness solver now distinguishes unresolved external runtime targets from external calls whose receiver is a modeled target state value. A configured receiver such as an asset held in contract state is recorded as discovered-but-runtime-verifiable rather than making the producer intrinsically unconstructible. The recursive setup planner blocks only unresolved runtime dependencies. This preserves fail-closed behavior for genuinely unknown targets while allowing ordinary target-configured transitions to proceed to execution, where deterministic tools can verify actual call behavior. Regression coverage was added for both classification and recursive setup.

Current head: a1683d66031b77df22eae319e6e53b10172e71b8.
Next frontier: resolve non-state-backed value producers such as interface/contract calls from compiler-linked declarations and target configuration, without turning interface names or function names into target-specific rules. The resulting producer must still carry unresolved dependencies until execution evidence verifies them.
